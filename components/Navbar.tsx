      ...
      <header className={`nav-shell ${scrolled ? "nav-shell-scrolled" : ""}`}>
        <div className="site-container">
          <div className="nav-inner">
            ...
            <button onClick={() => setOpen(true)} className="mobile-menu-btn" aria-label="Open menu">
      ...
      <div className={`mobile-drawer-wrap ${open ? "mobile-drawer-open" : ""}`}>
        ...
        <aside className="mobile-drawer">
          <div className="mobile-drawer-top">
            ...
            <button onClick={() => setOpen(false)} className="mobile-close" aria-label="Close menu">
              <X className="h-5 w-5" />
            </button>
          </div>
          ...
          <div className="mobile-nav-list">
            {links.map((link) => (
              <button
                key={link.id}
                onClick={() => go(link.id)}
                className={`mobile-nav-item ${active === link.id ? "mobile-nav-item-active" : ""}`}
              >
                {link.label}
              </button>
            ))}
