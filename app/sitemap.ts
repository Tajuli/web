...
export default function sitemap(): MetadataRoute.Sitemap {
  const now = new Date();
  const staticPages: MetadataRoute.Sitemap = ["/", "/blogs", "/case-studies", "/privacy-policy", "/terms", "/cookies", "/#services", "/#about", "/#contact"].map((path) => ({
  ...
  const blogPages = getAllBlogs().map((blog) => ({ url: absoluteUrl(`/blogs/${blog.slug}`), lastModified: new Date(toIsoDate(blog.publishedAt) || blog.publishedAt), changeFrequency: "monthly" as const, priority: blog.featured ? 0.9 : 0.75 }));
