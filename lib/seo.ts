...
import { absoluteUrl, pathFromUrl, siteConfig } from "./site";
...
export function createMetadata(input: SeoInput = {}): Metadata {
  ...
  return {
    ...
    openGraph: { type: input.type || "website", locale: siteConfig.locale, url: canonical, siteName: siteConfig.name, title: fullTitle, description, images: [{ url: image, width: 1200, height: 630, alt: fullTitle }], ...(input.type === "article" && { publishedTime: input.publishedTime, modifiedTime: input.modifiedTime || input.publishedTime, authors: input.authors || [siteConfig.name] }) },
